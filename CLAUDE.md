...
Watcher Python per Windows che sincronizza bidirezionalmente una cartella OneDrive aziendale con
una cartella Google Drive personale, gestendo file Office bloccati, percorsi lunghi e un'unità
virtuale SUBST per accorciare i percorsi. Puo' girare come processo interattivo o come servizio
Windows.
...
`.claude/skills/`. Lo standard di sistema completo è in `.claude/PROJECT-SYSTEM.md`, importato da
`E:\template-claude-developing` in fase di allineamento.

## Apprendimenti recenti

```
- [2026-07-09] Allineamento al sistema portabile di contesto eseguito su un repository esistente
  con 15 commit di storia; nessun segreto trovato nella storia né nei file tracciati.
```
